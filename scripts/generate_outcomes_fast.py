...
def generate_training_plots():
    print("Step 1: Generating Training Plots using mock history (fast mode)...")
    
    # Mock history for fast generation
    history = {
...
def generate_weather_plots():
    ...
    plt.figure(figsize=(8, 6))
    colors = ['green' if p < rain_threshold else 'tab:blue' for p in rain_prob] 
    ...
    for x, y in zip(hours, wind_speed):
        if y < wind_threshold:
            ...
            plt.text(x, 0.5, '✘', ha='center', color='red', fontsize=12, fontweight='bold') 

    
    plt.title('Figure 3(a). Wind speed threshold validation\nfor drift-safe spraying.')
    plt.xlabel('Forecast Horizon (hours)')
